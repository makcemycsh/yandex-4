...
export default class PointerHandler {
  constructor(selector) {
    ...
    this.$imgWrap = $('.JsImgWrapper', this.selector);
    this.$img = $('img', this.$imgWrap);
    this.$scroll = $('.JsScroll', this.selector);
    this.$zoom = $('.JsZoom', this.selector);
    this.$brightness = $('.JsBrightness', this.selector);
...
// Лимит для скролла изображения
  checkLim() {
    if (this.top >= this.topLim) this.top = this.topLim;
    if (this.top <= -this.topLim) this.top = -this.topLim;

    if (this.left >= this.leftLim) this.left = this.leftLim;
    if (this.left <= -this.leftLim) this.left = -this.leftLim;
  }
  ...
  moveScroll() {
    ...
    if (persent > end) persent = end;
    if (persent < start) persent = start;
